...
So far all our labels and comments are written in English, yet there is no explicit indication of that in the vocabulary. To address this, RDF uses the concept of **language tags**: after a string, you can add a tag (e.g., `@en` for English, or `@es` for Spanish) to stipulate its language.
